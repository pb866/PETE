...
! Time2 is local time in hours
    ...
    ! mcm 2013 from IUPAC 2009
    kapho2    = 5.20d-13*EXP(980.0/temp)

    ! kapno  : rcoo2    + no      = products
    ! mcm 2013 from IUPAC 2002
    kapno = 7.5d-12*EXP(290.0/temp)

    ! kro2no3: ro2      + no3     = products
    ! mcm 2013 from IUPAC 2008
    kro2no3   = 2.30d-12

    ! kno3al : no3      + rcho    = rcoo2   + hno3
    ! mcm 2013 from IUPAC 2007
    kno3al    = 1.4d-12*EXP(-1860.0/temp)

    ! kdec   : ro                 = products
    ! mcm 2013 from Atkinson 2007
    kdec      = 1.00d+06
...
!    kalkpxy=1.50d-14*EXP(-200.0/temp)*o2
    ...
    ! kmt08  : oh       + no2     = hno3
    ! iupac 2009 k8i different on MCM = 4.1d-11
    ...
    ! kmt12 oh + so2 + m = hoso2 + m
    ! mcm 2013 from iupac 2009
    k120   = 4.50d-31*m*(temp/300.0)**(-3.9)
    ...
    ! mcm 2013 from iupac 2009
    k130     = 2.50d-30*m*(temp/300.0)**(-5.5)
    k13i     = 1.80d-11
    kr13     = k130/k13i
    fc13     = 0.36
    ...
    ! mcm 2013 from iupac 2009
    k140     = 9.0d-05*EXP(-9690.0/temp)*m
    ...
    ! kmt15 oh + c2h4 + m = c3h4oh + m
    ! mcm 2013 from iupac 2005
    k150     = 8.60d-29*m*(temp/300.0)**(-3.1)
    k15i     = 9.0d-12*(temp/300.0)**(-0.85)
    kr15     = k150/k15i
    fc15     = 0.48
    ...
    ! kmt16  :  oh  +  c3h6 + m = c3h6oh + m
    ! mcm 2013 iupac 2005
    k160     = 8.00d-27*m*(temp/300.0)**(-3.5)
    ...
    ! kmt17 oh + c2h2 + m = c2h2oh + m
    ! mcm 2013 from iupac 2005
    k170     = 5.00d-30*m*(temp/300.0)**(-1.5)
    k17i     = 1.00d-12
    kr17     = k170/k17i
    fc17     = 0.17*EXP(-51.0/temp)+EXP(-temp/204.0)
    nc17     = 0.75-1.27*(dlog10(fc17))
    f17      = 10**(dlog10(fc17)/(1+(dlog10(kr17)/nc17)**2))
    kmt17    = (k170*k17i*f17)/(k170+k17i)

    ! kmt18 oh + ch3sch3
    ! mcm 2013 from iupac 2006
    kmt18 = 9.5d-39*o2*EXP(5270.0/temp)/(1+7.5d-29*o2*EXP(5610.0/temp))
...
!Halogens
                       if (jl .eq. 72)  j(1001)=0.!seval(n,theta,tmp, tmp2, b,c,d) ! HOBr
                       if (jl .eq. 73)  j(1002)=0.!seval(n,theta,tmp, tmp2, b,c,d) ! BrO
                       if (jl .eq. 74)  j(1003)=0.!seval(n,theta,tmp, tmp2, b,c,d) ! Br2
                       if (jl .eq. 50)  j(1004)=0.!seval(n,theta,tmp, tmp2, b,c,d) ! BrNO3->Br+NO3
                       if (jl .eq. 51)  j(1005)=0.!seval(n,theta,tmp, tmp2, b,c,d) ! BrNO3->BrO+NO2
                       if (jl .eq. 30)  j(1006)=0.!seval(n,theta,tmp, tmp2, b,c,d) ! ClNO3->Cl+NO3
                       if (jl .eq. 31)  j(1007)=0.!seval(n,theta,tmp, tmp2, b,c,d) ! ClNO3->ClO+NO2
